...
:ui_base_radius(0), ui_ind_radius(0), button_radius(13), parent(_parent), mode(DT_DIAL_UI_HOME){
...
void dt_dial_ui::make_vbo_shape(){
	...
	for(int i=0; i<=shape_step_num; i++){
		...
		if(i!=shape_step_num)shape_vbo_points.push_back(ofVec2f(((float)ui_template_radius+w)*cosf(rad),((float)ui_template_radius+w)*sinf(rad)));
		
		ofFloatColor color;
		float gray = 0.8;
		color.set(gray);
		shape_vbo_colors.push_back(color);
		if(i!=shape_step_num)shape_vbo_colors.push_back(color);
	}
...
void dt_dial_ui::make_vbo_param(){
	...
	for(int i=0; i<=param_step_num; i++){
		...
		if(i!=param_step_num)param_vbo_points.push_back(ofVec2f(((float)ui_template_radius+w)*cosf(rad),((float)ui_template_radius+w)*sinf(rad)));
...
//	ofFill();
//	ofSetColor(230, 220);
//	ofCircle(0, 0, ui_base_radius*1.3);
	
	switch (mode) {
		case DT_DIAL_UI_NONE: break;					// -1
		case DT_DIAL_UI_HOME: draw_HOME(); break;		//  0
		case DT_DIAL_UI_SPEED: draw_SPEED(); break;		//	1
		case DT_DIAL_UI_SHAPE: draw_SHAPE(); break;		//	2
		case DT_DIAL_UI_ROT: draw_ROT(); break;			//  3
		case DT_DIAL_UI_MUTE: draw_MUTE(); break;		//	4
		case DT_DIAL_UI_PRAM: if(bNeedParam)draw_PARAM(); break;		//  5
		default: break;
	}
...
void dt_dial_ui::draw_HOME(){
	...
	float p_radius = parent->data.rev_radius;
	float angle = 0;
	float step_angle = 50; // 200.0 / (float)num_home_button;
